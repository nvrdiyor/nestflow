...
export function renderAuth(root: HTMLElement, navigate: Nav, mode: Mode): void {
  ...
  <div class="auth-wrap">
    ...
    <div class="auth-card">
      ...
      <p class="sub js-sub">${isReg ? t('auth.createSub') : t('auth.loginSub')}</p>
      ...
      <div class="js-body">
        ...
        <div class="js-tg hidden">
          <div class="auth-or"><span>${t('auth.or')}</span></div>
          <button class="btn btn-tg js-tg-login" type="button">${TG_ICON}<span>${t('auth.tgLogin')}</span></button>
        </div>
  ...
  const showCodeStep = (ch: TgChallenge, note: string): void => {
    ...
    body.querySelector('.js-back')!.addEventListener('click', () => renderAuth(root, navigate, mode));
    codeInput.focus();
  };

  // "Continue with Telegram" appears only when the server has a bot configured.
  void getConfig().then((cfg) => {
    if (!cfg.telegramBot) return;
    const tgBlock = root.querySelector<HTMLElement>('.js-tg');
    if (!tgBlock) return;
    tgBlock.classList.remove('hidden');
    const btn = tgBlock.querySelector<HTMLButtonElement>('.js-tg-login')!;
    btn.addEventListener('click', async () => {
      errorEl().classList.add('hidden');
      btn.disabled = true;
      try {
        showCodeStep(await telegramStart(), t('auth.tgLoginNote'));
      } catch (err) {
        showError(err);
        btn.disabled = false;
      }
  ...
  root.querySelector<HTMLFormElement>('.js-form')!.addEventListener('submit', async (e) => {
    ...
    try {
      if (isReg) {
        ...
        if ('pending' in res) {
          showCodeStep(res.pending, t('auth.tgRegNote'));
          return;
        }
      } else {
